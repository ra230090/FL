...
for i in range(1,testround):
    ...
    #get input
    f = open('%s/problemIO/%d/%d_input%d.txt'%(abpath,file_ID,file_ID,i), 'r')
    data = f.read()
    f.close()
    #feed the input
    ...
    ###
    os.system('diff %d_useroutput%d.txt %s/problemIO/%d/%d_output%d.txt >output_same%d.txt'%(file_ID,i,abpath,file_ID,file_ID,i,i))
    os.system('diff -w -B %d_useroutput%d.txt %s/problemIO/%d/%d_output%d.txt >output_same_ignore%d.txt'%(file_ID,i,abpath,file_ID,file_ID,i,i))
